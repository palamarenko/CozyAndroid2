...
class ShapeImageView : AppCompatImageView {
    ...
    private fun init(attrs: AttributeSet?) {
        if (attrs != null) {
            val arr: TypedArray =
                context.obtainStyledAttributes(attrs, R.styleable.ShapeImageView)
    ...
    private fun getViewShapeDrawable(arr: TypedArray): ShapeDrawableFactory {
        ...
        drawable.shape = SHAPE.values()[arr.getInt(R.styleable.ShapeImageView_back_shape, 0)]
        val colorBack = arr.getString(R.styleable.ShapeImageView_back_shape_color)
        if (colorBack != null) {
            drawable.colorBack = Color.parseColor(colorBack)
        }
        val startColor =
            arr.getString(R.styleable.ShapeImageView_gradient_start_color)
        if (startColor != null) {
            drawable.startColor = Color.parseColor(startColor)
        }
        val endColor = arr.getString(R.styleable.ShapeImageView_gradient_end_color)
        if (endColor != null) {
            drawable.endColor = Color.parseColor(endColor)
        }
        val radius = arr.getDimensionPixelSize(R.styleable.ShapeImageView_corners_radius, -1)
        if (radius >= 0) {
            drawable.radius = radius
        }
        val border =
            arr.getBoolean(R.styleable.ShapeImageView_back_shape_border, false)
        drawable.isBorder = border
        val borderWidth =
            arr.getDimensionPixelOffset(R.styleable.ShapeImageView_back_shape_border_width, 0)
        if (borderWidth > 0) {
            drawable.borderWidth = borderWidth
        }
        val gradientDirection = arr.getInt(R.styleable.ShapeImageView_gradient_direction, 0)
